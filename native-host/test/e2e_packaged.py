#!/usr/bin/env python3
"""Packaged E2E: fixture server -> release native host -> release
engine-host exe -> bytes on disk. Verifies the shipped binaries,
not the dev toolchain.

Run from the repo root:
  python native-host/test/e2e_packaged.py
"""
import hashlib
import json
import os
...
REL = os.path.join(ROOT, "release", "jjamidm-rc")
HOST_EXE = os.path.join(REL, "jjamidm_native_host.exe")
ENGINE_EXE = os.path.join(REL, "jjamidm-engine-host.exe")
DART = os.path.join(ROOT, "..", ".tools", "dart-sdk", "bin",
                    "dart.exe")
...
with open(cfg_path, "w") as f:
    json.dump({
        ...
        "engineCommand": [ENGINE_EXE, "--temp-root", tmp],
        "engineCwd": REL,
        "downloadDir": dl,
    }, f)
...
if failures:
    ...
    sys.exit(1)
print("E2E passed: packaged host -> packaged engine -> disk")
