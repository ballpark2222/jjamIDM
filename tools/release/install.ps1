# FreeDM one-shot installer (dev/RC channel).
#   1. Writes the native-messaging host manifest pointing at the
#      packaged freedm_native_host.exe.
#   2. Writes %APPDATA%\FreeDM\native-host.json (engine command,
#      download dir) — the host cold-launches freedm-engine-host.exe.
...
param(
  [string]$RcDir = "$PSScriptRoot\..\..\release\freedm-rc",
  [string]$ExtensionId = 'lfgbaljhboceihkklkifpfbengheamdo',
  [string]$DownloadDir = "$env:USERPROFILE\Downloads\FreeDM"
)
$ErrorActionPreference = 'Stop'
$RcDir = (Resolve-Path $RcDir).Path
$sep = [IO.Path]::DirectorySeparatorChar

$hostExe  = "$RcDir\freedm_native_host.exe"
$engineExe = "$RcDir\desktop\freedm-engine-host.exe"
...
$configDir = "$env:APPDATA\FreeDM"
$dataDir = "$env:LOCALAPPDATA\FreeDM"

# 1. host manifest — absolute exe path + pinned extension origin.
$manifest = [ordered]@{
  name = 'ai.devin.freedm'
  description = 'FreeDM native messaging host'
  path = $hostExe
  type = 'stdio'
...
foreach ($browser in @('Google\Chrome', 'Microsoft\Edge')) {
  $key = "HKCU:\Software\$browser\NativeMessagingHosts\ai.devin.freedm"
